...
public:
  ...
  std::string ConcatenateAESType(unsigned char* keyAddr, int type);
  // Find AES Keys in exe buffer.
  bool FindAESKeys(char* buffer, unsigned long long size);
  ...
  std::pair<double, std::vector<unsigned long long>> FindMaxElements(std::vector<double> const& v);
...
private:
  ...
  std::vector<unsigned char*> Find(const char* pattern, unsigned char* rangeStart = NULL, unsigned char* rangeEnd = NULL);
...
public:
  ...
  std::vector<unsigned long long> m_MostLikelyKey{};
